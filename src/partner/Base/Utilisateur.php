...
abstract class Utilisateur implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\partner\\Map\\UtilisateurTableMap';
    ...
    /**
     * Initializes internal state of partner\Base\Utilisateur object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\partner\Utilisateur The current object (for fluent API support)
     */
    public function setUtilisateurID($v)
    {
        ...
        if ($this->utilisateurid !== $v) {
            ...
            $this->modifiedColumns[UtilisateurTableMap::COL_UTILISATEURID] = true;
        }
    ...
    public function setMotDePasse($v)
    {
        ...
        if ($this->mot_de_passe !== $v) {
            $this->mot_de_passe = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_MOT_DE_PASSE] = true;
        }
    ...
    public function setCreatedAt($v)
    {
        ...
        if ($this->created_at !== null || $dt !== null) {
            if ($this->created_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->created_at->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[UtilisateurTableMap::COL_CREATED_AT] = true;
            }
    ...
    public function setLastUpdated($v)
    {
        ...
        if ($this->last_updated !== null || $dt !== null) {
            if ($this->last_updated === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->last_updated->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[UtilisateurTableMap::COL_LAST_UPDATED] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UtilisateurTableMap::translateFieldName('Login', TableMap::TYPE_PHPNAME, $indexType)];
            $this->login = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UtilisateurTableMap::translateFieldName('MotDePasse', TableMap::TYPE_PHPNAME, $indexType)];
            $this->mot_de_passe = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UtilisateurTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UtilisateurTableMap::translateFieldName('LastUpdated', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 6; // 6 = UtilisateurTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\partner\\Utilisateur'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_UTILISATEURID)) {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_MOT_DE_PASSE)) {
            $modifiedColumns[':p' . $index++]  = 'mot_de_passe';
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = 'created_at';
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_LAST_UPDATED)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\partner\Utilisateur
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\partner\Utilisateur The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_UTILISATEURID)) {
            $criteria->add(UtilisateurTableMap::COL_UTILISATEURID, $this->utilisateurid);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_NOM)) {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_MOT_DE_PASSE)) {
            $criteria->add(UtilisateurTableMap::COL_MOT_DE_PASSE, $this->mot_de_passe);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_CREATED_AT)) {
            $criteria->add(UtilisateurTableMap::COL_CREATED_AT, $this->created_at);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_LAST_UPDATED)) {
            $criteria->add(UtilisateurTableMap::COL_LAST_UPDATED, $this->last_updated);
        }
    ...
    /**
     * Generic method to set the primary key (utilisateurid column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \partner\Utilisateur (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \partner\Utilisateur Clone of current object.
     * @throws PropelException
     */
    ...
    public function setUserprestations(Collection $userprestations, ConnectionInterface $con = null)
    {
        ...
        foreach ($userprestationsToDelete as $userprestationRemoved) {
            $userprestationRemoved->setUtilisateur(null);
        }
    ...
    protected function doAddUserprestation(ChildUserprestation $userprestation)
    {
        ...
        $userprestation->setUtilisateur($this);
    }
    ...
    public function removeUserprestation(ChildUserprestation $userprestation)
    {
        if ($this->getUserprestations()->contains($userprestation)) {
            ...
            $userprestation->setUtilisateur(null);
        }
     ...
     * @return ObjectCollection|ChildUserprestation[] List of ChildUserprestation objects
     */
    public function getUserprestationsJoinPrestation(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUserprestationQuery::create(null, $criteria);
    ...
    public function setDevis(Collection $devis, ConnectionInterface $con = null)
    {
        ...
        foreach ($devisToDelete as $deviRemoved) {
            $deviRemoved->setUtilisateur(null);
        }
     ...
     * @return ObjectCollection|ChildDevi[] List of ChildDevi objects
     */
    public function getDevisJoinClient(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildDeviQuery::create(null, $criteria);
